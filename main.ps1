...
Invoke-Parallel    -InputObject $(get-content G:\chj\disable-chrome-gpu\tilllist.txt)  -SleepTimer 2 -ScriptBlock {
$testeconnection = Test-Connection "$_"  -ErrorAction SilentlyContinue
if ($testeconnection -eq $null){ echo  "$_ is offline" 
}
else{
#back up the older file
$backupfile = Test-Path \\"$_"\c$\systools\OB\launch.cmd.backupforgpu
if($backupfile -eq $false){
cp \\"$_"\c$\systools\OB\launch.cmd  \\"$_"\c$\systools\OB\launch.cmd.backupforgpu
echo "$_ is backing up"
}
else{echo "$_ is backedup"}
#check if parameter is updated 
[string]$file = (Get-Content  \\"$_"\c$\systools\OB\launch.cmd) -join "`r`n"
$checkresult = $file.Contains('--disable-gpu')
#add parameter if false
if($checkresult -eq $false){
$file = $file -replace 'kiosk' , 'kiosk --disable-gpu'
$file
echo "$_ is updating" 
$file | Out-File -Encoding ascii  \\"$_"\c$\systools\OB\launch.cmd
